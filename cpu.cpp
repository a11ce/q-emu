...
// this is not technically correct, needs the 1/sqrt(2) coeffec. but fine for
// testing purposes
...
public:
  struct Interface {
    ...
    virtual int targetWire() const { return -1; }

    virtual void print() const { printMatrix(toMatrix()); }
  };
  std::shared_ptr<const Interface> _p;

public:
  Gate(Interface *p) : _p(p) {}
  string type() const { return _p->type(); }
  Matrix toMatrix() const { return _p->toMatrix(); }
  int wire() const { return _p->wire(); }
  int targetWire() const { return _p->targetWire(); }
  void print() const { _p->print(); }
};
...
public:
  ...
  Matrix toMatrix() const override { return matrix; };

  int wire() const override { return wireIdx; }
};
...
public:
  ...
  string type() const override { return "controlled"; }

  Matrix toMatrix() const override { return matrix; }

  int wire() const override { return wireIdx; }

  int targetWire() const override { return targetWireIdx; }
};
...
public:
  ...
  TimeSlice(vector<Gate> _gates, int _nQubits)
  ...
  Matrix processGates(vector<Gate> _gates, int num_of_wires) {
    ...
    if (!control_queue.empty()) {
      ...
      return collapseControlledMatrixVector(M_Vec, g.toMatrix(), g.wire(),
                                            g.targetWire());
    } else {
  ...
  Matrix toMatrix() { return processGates(gates, nQubits); }
};
...
public:
  ...
  Circuit(int _nQubits) { int nQubits = _nQubits; }
...
int main(void) {
  ...
  Circuit Circ_1({TimeSlice({new SingleQubitGate(X, 0)}, 2),
                  TimeSlice({new SingleQubitGate(X, 1)}, 2)},
                 2);
  ...
  StateVector Ket01 = {Complex(0, 0), Complex(1, 0), Complex(0, 0),
                       Complex(0, 0)};
  testStateVectorsEqual(
      Circ_2.applyTransformationToSlice(makeStateVector(2), 0), Ket01,
      "(I ⊗ X)|00>))");
  testStateVectorsEqual(
      Circ_2.applyTransformationToSlice(makeStateVector(2), 1), Ket01,
      "(CX(0,1))|01>))");
  testStateVectorsEqual(
      Circ_2.applyTransformationToSlice(makeStateVector(2), 2), Ket11,
      "(CX(1,0)|01>))");
}
